...
def init_session_state():
    session_vars = {
        ...
        'processing_chunk_progress': 0,  # New state for chunk progress
        'processing_chunk_remaining': "Calculating..."  # New state for remaining time
    }
...
def format_entries_chunked(text, status_text):
    ...
    chunks = ['\n\n'.join(entries[i:i+100]) for i in range(0, len(entries), 100)]
...
{{chunk}}"""
    ...
    for i, chunk in enumerate(chunks):
        ...
        st.session_state.processing_chunk_progress = progress
        progress_bar.progress(progress)
        ...
        if i > 0:  # Only estimate after first chunk
            ...
            st.session_state.processing_chunk_remaining = format_time(estimated_remaining)
            status_text.text(
                f"Processing chunk {i+1}/{len(chunks)} "
                f"({progress}%) - "
                f"Estimated time remaining: {st.session_state.processing_chunk_remaining}"
            )
        else:
            status_text.text(f"Processing chunk {i+1}/{len(chunks)} ({progress}%)")
    ...
    if st.session_state.app_mode == "📝 Create Entries":
            ...
            if st.button("✨ Format Entries"):
                if raw_text.strip():
                    ...
                    progress_container.markdown(
                        f"**Processing:** 0% completed\n"
                        f"**Estimated Time Remaining:** Calculating..."
                    )
        ...
        if st.session_state.processing:
            ...
            while st.session_state.processing:
                progress_container.markdown(
                    f"**Processing:** {st.session_state.processing_chunk_progress}% completed\n"
                    f"**Chunk:** {st.session_state.current_chunk}/{st.session_state.total_chunks}\n"
                    f"**Estimated Time Remaining:** {st.session_state.processing_chunk_remaining}"
                )
                time.sleep(0.5)
