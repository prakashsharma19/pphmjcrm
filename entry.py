...
def init_session_state():
    session_vars = {
        ...
        'save_progress': 0,
        'save_status': "",
        'legacy_duplicates_check': True  # Flag to enable legacy duplicate checking
    }
...
def check_legacy_duplicates(name, email):
    """Check for duplicates in pre-existing entries (before author_keys implementation)"""
    if not name or not email:
        return False
    ...
    # Search across all journals and files
    for journal in db.collection("journals").stream():
        for file in db.collection("journals").document(journal.id).collection("files").stream():
            ...
            for entry in entries:
                entry_name, entry_email = extract_author_email(entry)
                if entry_name and entry_email:
                    if (name.lower() == entry_name.lower() and 
                        email.lower() == entry_email.lower()):
                        return True
    return False
...
def check_duplicates(new_entries):
    ...
    for i, entry in enumerate(new_entries):
        ...
        # Then check against existing entries using both methods
        is_dup = False
        source = ""
        
        # First check with author_keys (fast)
        if is_duplicate(name, email):
            is_dup = True
            source = "author_keys"
        # If not found and legacy check is enabled, do deeper search
        elif st.session_state.legacy_duplicates_check and check_legacy_duplicates(name, email):
            is_dup = True
            source = "legacy_entries"
                        ...
                        if duplicates:
                            ...
                            with st.expander("🔍 Duplicate Details"):
                                for key, dup_list in duplicates.items():
                                    ...
                                    # Show breakdown of duplicate sources
                                    current_batch = [d for d in dup_list if d.get("source") == "current_batch"]
                                    author_keys = [d for d in dup_list if d.get("source") == "author_keys"]
                                    legacy = [d for d in dup_list if d.get("source") == "legacy_entries"]
                                    
                                    if current_batch:
                                        st.write(f"- Duplicates within this upload: {len(current_batch)}")
                                    if author_keys:
                                        st.write(f"- Already exists in database (fast check)")
                                    if legacy:
                                        st.write(f"- Already exists in database (legacy check)")
