...
st.set_page_config(page_title="PPH CRM - Testings2", layout="wide", initial_sidebar_state="expanded")
init_session_state()
...
if st.session_state.keep_alive_interval is None:
    st.session_state.keep_alive_interval = threading.Thread(target=keep_alive, daemon=True)
    st.session_state.keep_alive_interval.start()
    ...
    try:
        ...
        for i, entry in enumerate(entries):
            ...
            if is_duplicate(name, email):
                duplicates_found += 1
                # Get duplicate info
                doc = db.collection("author_keys").document(f"{name.lower()}_{email.lower()}").get()
                if doc.exists:
                    dup_data = doc.to_dict()
                    if email not in duplicates_info:
                        duplicates_info[email] = []
                    duplicates_info[email].append({
                        "entry": entry,
...
def format_entries_chunked(text, status_text):
    ...
    st.session_state.total_entries_count = len(entries)
    
    if not entries:
        return ""
    
    # Split entries into chunks of 50 entries each
    chunks = ['\n\n'.join(entries[i:i+50]) for i in range(0, len(entries), 50)]
    st.session_state.total_chunks = len(chunks)
    st.session_state.current_chunk = 0
    formatted_parts = []
    st.session_state.formatted_entries_count = 0
...
{{chunk}}"""
    ...
    for i, chunk in enumerate(chunks):
        ...
        if i > 0:  # Only estimate after first chunk
            ...
            status_text.text(
                ...
                f"Entries processed: {i * 50} of {st.session_state.total_entries_count}"
            )
        ...
        try:
            ...
            if response.text:
                ...
                st.session_state.formatted_entries_count += len(formatted_chunk.split('\n\n'))
                ...
                with progress_container.container():
                    ...
                    **Processing Status:**
                    - Progress: {progress}%
                    - Chunks processed: {i+1}/{len(chunks)}
                    - Total entries processed: {st.session_state.formatted_entries_count}/{st.session_state.total_entries_count}
                    - Estimated time remaining: {format_time(estimated_remaining) if i > 0 else 'Calculating...'}
                    """)
    ...
    with progress_container.container():
        st.success(f"""
        **Processing Complete!**
        - Total time: {format_time(processing_time)}
        - Total entries processed: {len(final_text.split('\n\n'))}
        - Success rate: {round(len(final_text.split('\n\n')) / st.session_state.total_entries_count * 100, 2)}%
        """)
    
    return final_text

def save_to_database_thread(entries, journal, filename, status_text):
    """Thread function for saving entries to database"""
    st.session_state.processing = True
    st.session_state.save_complete = False
    unique_entries, duplicates_info = save_entries_with_progress(entries, journal, filename, status_text)
...
def test_service_connections(max_retries=3, retry_delay=2):
    """Test connections to AI and Firebase services with retries"""
    # Test AI
...
def show_entry_module():
    ...
    if st.session_state.app_mode == "📝 Create Entries":
        ...
        if st.session_state.show_formatting_results and st.session_state.formatted_text:
            ...
            st.write(f"**Total Entries Processed:** {len(st.session_state.formatted_text.split('\n\n'))}")
            ...
            if st.session_state.create_entry_stage == 'save':
                ...
                if st.button("💾 Save to Database"):
                    if selected_journal and filename:
                        ...
                        # Start save process in a separate thread
                        st.session_state.save_thread = threading.Thread(
                            target=save_to_database_thread,
                            args=(st.session_state.formatted_entries, selected_journal, filename, status_text)
                        )
                        st.session_state.save_thread.start()
                        
                        # Show processing indicator
                        with st.spinner("Saving entries to database..."):
                            while st.session_state.save_thread.is_alive():
                                time.sleep(0.1)
                        
                        # After save completes
                        if st.session_state.save_complete:
        ...
        st.header("📤 Upload Formatted Entries")
        
        # File upload option
        uploaded_file = st.file_uploader("📄 Upload formatted entries file", type=["txt"])
        if uploaded_file:
            uploaded_entries = process_uploaded_file(uploaded_file)
            if uploaded_entries:
                st.session_state.uploaded_entries = uploaded_entries
                st.write(f"Found {len(uploaded_entries)} entries in the uploaded file")
                st.text_area("Uploaded Entries Preview (first 5 entries)", 
                            value="\n\n".join(uploaded_entries[:5]), 
                            height=200,
                            disabled=True)
        ...
        if st.session_state.uploaded_entries:
            ...
            if st.button("💾 Save to Database"):
                if selected_journal and filename:
                    ...
                    st.session_state.save_thread = threading.Thread(
                        target=save_to_database_thread,
                        args=(st.session_state.uploaded_entries, selected_journal, filename, status_text)
                    )
                    st.session_state.save_thread.start()
                    ...
                    for i, file in enumerate(files[:st.session_state.files_to_display]):
                        try:
                            ...
                            with expander:
                                ...
                                with col2:
                                    # Direct download without additional button
                                    content, count = download_entries(selected_journal, file['name'])
                                    if content:
                                        st.download_button(
                                            "📥 Download",
                                            content,
                                            file_name=f"{file['name']} ({count} entries).txt",
                                            mime="text/plain",
                                            key=f"dl_file_{i}"
                                        )
